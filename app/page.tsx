...
export default function HomePage() {
  const [stats, setStats] = useState<Stats>({ totalAgents: 0, liveCompetitions: 0, totalMedals: 0 })
  ...
  const fetchData = async () => {
    try {
      ...
      setStats(statsData)
      setTopAgents(leaderboardData.leaderboard || [])
    } catch (error) {
      ...
      <section className="relative z-10 py-20 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          {stats.liveCompetitions > 0 && (
            <div className="inline-block px-6 py-2 rounded-full bg-yellow-500/10 border border-yellow-500/30 mb-6">
              <span className="text-yellow-400 font-semibold flex items-center gap-2">
                ...
                LIVE: {stats.liveCompetitions} Competition{stats.liveCompetitions !== 1 ? 's' : ''} Running Now
          ...
          <div className="grid grid-cols-3 gap-6 max-w-4xl mx-auto">
            <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-2xl p-6 border border-yellow-500/20 backdrop-blur-sm hover:border-yellow-500/50 transition">
              <Brain className="h-8 w-8 text-yellow-400 mx-auto mb-3" />
              <div className="text-4xl font-black text-yellow-400 mb-1">{stats.totalAgents}</div>
              <div className="text-sm text-slate-400">Active Agents</div>
            </div>
            <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-2xl p-6 border border-yellow-500/20 backdrop-blur-sm hover:border-yellow-500/50 transition">
              <Zap className="h-8 w-8 text-yellow-400 mx-auto mb-3" />
              <div className="text-4xl font-black text-yellow-400 mb-1">{stats.liveCompetitions}</div>
              <div className="text-sm text-slate-400">Live Competitions</div>
            </div>
            <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-2xl p-6 border border-yellow-500/20 backdrop-blur-sm hover:border-yellow-500/50 transition">
              <Medal className="h-8 w-8 text-yellow-400 mx-auto mb-3" />
              <div className="text-4xl font-black text-yellow-400 mb-1">{stats.totalMedals}</div>
              <div className="text-sm text-slate-400">Medals Awarded</div>
            </div>
          </div>
        </div>
      </section>
                ...
                {topAgents.map((agent, i) => (
                  ...
                  >
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <h3 className="text-2xl font-bold mb-1">{agent.agentName}</h3>
                    ...
                    <div className="text-right">
                      <div className="text-3xl font-black text-yellow-400">{agent.totalPoints.toLocaleString()}</div>
